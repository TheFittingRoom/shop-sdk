...
import {
  AvatarNotCreated,
  AvatarNotReady,
  ErrorOutsideRecommendedSizes,
  ErrorResponse,
  SizeIDOutsideRecommendedRange,
} from './errors'
...
const InitShop = (firebaseInstance: any, id: number): types.Shop => {
  ...
  const AwaitColorwaySizeAssetFrames = async (
    user: types.FirebaseUser,
    colorwaySizeAssetSKU: string,
  ): Promise<types.TryOnFrames> => {
    return new Promise((resolve, reject) => {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        GetColorwaySizeAssetFrames(user, colorwaySizeAssetSKU)
          .then((frames) => {
            unsubscribe()
  ...
  const GetRecommendedSizes = async (
    user: types.FirebaseUser,
    BrandStyleID: string,
  ): Promise<SizeRecommendation | ErrorResponse> => {
    return new Promise((resolve, reject) => {
      if (notLoggedIn(user, reject)) return
      Fetcher.Get(user, `/styles/${BrandStyleID}/recommendation`, null)
        .then((r: Response) => {
          r.json()
            .then((data) => {
              resolve(data)
  ...
  const GetColorwaySizeAssets = async (
    style_id?: number,
    skus?: string[],
  ): Promise<Map<number, types.FirestoreColorwaySizeAsset>> => {
  ...
  const GetColorwaySizeAssetFrames = async (
    user: types.FirebaseUser,
    colorwaySizeAssetSKU: string,
  ): Promise<types.TryOnFrames> => {
    return new Promise((resolve, reject) => {
  ...
  const RequestColorwaySizeAssetFrames = async (
    user: types.FirebaseUser,
    ...
    return new Promise((resolve, reject) => {
      if (notLoggedIn(user, reject)) return
      Fetcher.Post(user, `/colorway-size-assets/${colorwaySizeAssetID}/frames`, null)
        .then(() => {
          resolve()
        })
  ...
  const TryOn = (
    user: types.FirebaseUser,
    colorwaySizeAssetSKU: string,
  ): Promise<types.TryOnFrames | (() => Promise<types.TryOnFrames>)> => {
              ...
              new Promise((resolve, reject) => {
                ...
                GetColorwaySizeAssets(null, [colorwaySizeAssetSKU])
                  ...
                  .catch((error: Error | ErrorOutsideRecommendedSizes) => {
                    ...
                    GetStyles([styleIDCache], [])
                      .then((styles: Map<number, types.FirestoreStyle>) => {
                        ...
                        recommendedSizeResponse.recommended_size =
                        ...
                        for (const available_size_id of errorOutsideRecommended.available_size_ids) {
                          if (
                            style.sizes[available_size_id] &&
                            style.sizes[available_size_id].size != recommendedSizeResponse.recommended_size
                          ) {
                            recommendedSizeResponse.available_sizes.push(
                              style.sizes[available_size_id].label || style.sizes[available_size_id]?.size,
                            )
                          }
                        }
